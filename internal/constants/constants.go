...
const (
	...
	AWSCredentialsKey           = "credentials"                               // key to expect for sts-formatted secret
	AWSRoleSessionName          = "cluster-logging"                           // identifier for role logging session
	AWSWebIdentityTokenName     = "sts-sa-token"                              // token name for pod
	AWSWebIdentityTokenMount    = "/var/run/secrets/openshift/serviceaccount" // default location for token volume mount
