...
# All credit to Brett Jones author of create-aws-cluster.sh
# most of this is taken directly from his script
# Purpose: Provide a simple/default set of instructions for easily creating an OpenShift cluster in sts mode
...
create_logging_role() {
#  request_dir="credrequests$(date +'%m%d')"
...
post_install() {
  ...
  echo -e "\n\n--- Setup CLI ---\n"
  echo -e "\nFirst run: ${AQUA}export KUBECONFIG=${CLUSTER_DIR}/auth/kubeconfig${NC}"
  echo -e "And login with: ${AQUA}oc login -u kubeadmin -p \$(cat ${CLUSTER_DIR}/auth/kubeadmin-password)${NC}"

  echo -e "\n\n--- Configure log forwarding via cloudwatch roles (IRSA) ---\n"
  echo -e "\nRun this script again '${ME}' with the flag -l or --logging-role, creating the role and logforwarder resources"

  echo -e "\n\n--- Cleanup Commands ---\n"
  echo -e "To clean up ccoctl resources after destroying cluster..."
  echo -e "Run this: ${GREEN}ccoctl aws delete --name=${CCO_RESOURCE_NAME} --region=${REGION}${NC}"

  echo -e "\nAlternatively, you run this script '${ME}' with the flag --cleanup"

  echo -e "\nTo clean up cloudwatch log groups..."
  echo -e "Run this: ${GREEN}aws logs describe-log-groups --query 'logGroups[?starts_with(logGroupName,\`${CCO_RESOURCE_NAME}\`)].logGroupName' --region ${REGION} --output text ${NC}"
  echo -e "And finally: ${GREEN}aws logs delete-log-group --region ${REGION} --log-group-name ${CCO_RESOURCE_NAME}- ${NC}"
...
# carried over from create-aws-cluster.sh
# ---
